...
setParameters(){
	if [ "$ENVIRONMENT" == "preview" ]; then
		...
		SOLR_PORT=""
		ALVIN_PORT=""
		IDPLOGIN_OPTIONS="JAVA_OPTS=-Dtoken.logout.url=http://alvin-login-test:8080/alvin/login/rest/authToken/" 
		IDPLOGIN_PORT=""
		LOGIN_PORT=""
...
startPostgresql() {
    ...
    docker run -d --name alvin-postgresql$ENV_SUFFIX \
     ...
     -e DATA_DIVIDERS="cora jsClient alvin alvinData alvinPreview" \
