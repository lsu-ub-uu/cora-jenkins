docker stop diva-cora-docker-postgresql-test diva-fitnesse-test diva-therest-test diva-solr-test diva-apptokenverifier-test diva-gatekeeper-test diva-idplogin-test diva-cora-postgresql-test diva-cora-fedora-test diva-synchronizer-test diva-docker-index-test && echo nothingToSeeMoveOnToNextCommand
docker rm diva-cora-docker-postgresql-test diva-fitnesse-test diva-therest-test diva-solr-test diva-apptokenverifier-test diva-gatekeeper-test diva-idplogin-test diva-cora-postgresql-test diva-cora-fedora-test diva-synchronizer-test diva-docker-index-test && echo nothingToSeeMoveOnToNextCommand
...
#synchronizer
docker run --net=diva-cora-test --name diva-synchronizer-test -e "JAVA_OPTS=-DapptokenVerifierURL=http://diva-apptokenverifier-test:8080/apptokenverifier/ -DbaseURL=http://diva-therest-test:8080/diva/rest/ -DuserId=${USER_ID} -DappToken=${AUTH_TOKEN}" -d cora-docker-synchronizer:1.0-SNAPSHOT
#fitnesse
...
sleep 20
#fedora
docker run --network-alias=diva-docker-fedora --net=diva-cora-test --restart always --name diva-cora-fedora-test --link diva-cora-postgresql-test:postgres-fcrepo -d diva-cora-docker-fedora-3.2.1:1.1-SNAPSHOT
...
#db with diva data
docker run --net=diva-cora-test --restart always \
-e POSTGRES_DB=diva \
